...
pthread_t stabilize_thread = (pthread_t) NULL;
...
// This thread might need to be cancelled whenever succ changes, idk
...
void server_run(server *srv) {
     ...
     */
     //n7ot function stabilize f server.h w namlelha implementation f peer.c eli nzidha f header server.c w nestamal variable global mta3 peer.c
     //include thread.h
     //while true tabath stabilize w slepp 2s hadha kol b thread create
     //void stabilize(void) w n7otha f thread_create nestamal ken succ f variable global
     //manestamalch forward nestamal send all
    //kifeh bech nabath: lezem nzidou l liste clients
    // Create thread that sends stabilize messages
    ...
    int i, ready;
    pthread_create(&stabilize_thread, NULL, send_stabilize, srv);
    while (srv->active) {
