...
void stack_push(stack* astack, float value) {
    /* HIER implementieren */
    stack_element* staack=malloc(sizeof(stack_element));
        staack->value=value;
        staack->next=NULL;
    if (astack->top==NULL){
        astack->top=staack;
...
float stack_pop(stack* astack)
{
    ...
    if(astack->top==NULL){
        ...
        stack_element* previous=astack->top;
...
{/* HIER implementieren */
    if(is_add(token)){
      float part1=stack_pop(astack);
      float part2=stack_pop(astack);
      stack_push(astack,part1+part2);
      return;
       }
     ...
     float part1 = atof(token);
     stack_push(astack,part1);
     }
